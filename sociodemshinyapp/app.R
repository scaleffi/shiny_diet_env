...
library(DT)
library(rsconnect) # to connect to the Posit account for deployment
library(shiny)
library(scales)
library(ggthemes)
...
myserver <- function(input, output) {
  ...
  source("server_files/server_radar_envitm.R", local = TRUE)
}

# Execute and launch the app ----------------------------------------------
sociodemapp <- shinyApp(ui = myUI, server = myserver)

# Execute and launch the app in showcase mode -----------------------------
# runApp(myapp
#        #"showcase" mode displays content of the dashboard and code behind it
